...
int main() {
    Mat inputImage = imread("image.jpg"); //because erosion or dilusion is for GRAY or Binary Images.
    if (inputImage.empty()) {
    ...
    #pragma omp parallel 
    {
        ...
        {
            erode(imageParts[i], imageParts[i], getStructuringElement(MORPH_RECT, Size(15, 15)));
        }
    }
    ...
    cout << "\nErosion Completed Successfully, image saved.";
    imwrite("eroded.png", combinedImage);
